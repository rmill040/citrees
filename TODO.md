...
Early stopping is treated as a computational heuristic. Paper-facing p-value claims are made only in fixed-$B$ mode.
See `paper/docs/paper.md` (Appendix A.5).

**If we include early stopping in the paper at all:** support it via calibration figures (not optional-stopping
theorems) using `paper/scripts/theory/generate_sequential_stopping_calibration.py`.
