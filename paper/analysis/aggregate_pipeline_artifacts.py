...
This script:
1. Builds the method_id set from paper/benchmark/pipeline/config.py
...
Usage:
    ...
    uv run python paper/analysis/aggregate_pipeline_artifacts.py \
        --local-dir ../data --task all

    # From S3:
    S3_BUCKET=my-bucket uv run python paper/analysis/aggregate_pipeline_artifacts.py \
        --task classification
...
def list_s3_objects(
    ...
    *,
    ...
    from paper.benchmark.adapters import get_s3_client
...
def main():
    ...
    output_files = {
        ("rankings", "classification"): "cache/clf_rankings.parquet",
        ("rankings", "regression"): "cache/reg_rankings.parquet",
    ...
    for task in tasks:
        ...
        for stage in stages:
            ...
            if args.local_dir is not None:
                ...
                from paper.benchmark.adapters import get_s3_bucket
                from paper.benchmark.config.constants import AWS_REGION
