...
Usage:
    ...
    uv run python paper/analysis/analyze_synthetic_ground_truth.py \
...
def _build_grid_method_ids(task: str) -> set[str]:
    """Return method IDs in the current synthetic experiment grid for one task."""
    grid = ExperimentGrid.from_cli(task, source="synthetic")
    return {config.method.label for config in grid}
