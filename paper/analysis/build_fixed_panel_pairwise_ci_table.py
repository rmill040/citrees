...
Usage:
  uv run python paper/analysis/build_fixed_panel_pairwise_ci_table.py
"""
...
def build_fixed_panel_pairwise_ci(*, n_boot: int = DEFAULT_N_BOOT) -> pd.DataFrame:
    """Summarize paired CIF-vs-baseline deltas on the 14-dataset benchmark."""
...
def main() -> None:
    """Build and save the 14-dataset benchmark uncertainty summary."""
    parser = argparse.ArgumentParser(
        description="Build paired uncertainty summary for CIF on the 14-dataset benchmark"
    )
    parser.add_argument(
        "--n-boot",
        type=int,
