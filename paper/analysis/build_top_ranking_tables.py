...
metadata with known informative feature indices. The raw ranking parquets are
local provenance files under `paper/results/cache/` and are not tracked.
...
from paper.analysis.analyze_synthetic_ground_truth import (
...
RANKINGS_DIR = RESULTS_DIR / "cache"

TASK_CONFIG: Final[dict[str, TopRankingTaskConfig]] = {
    "classification": {
        "rankings_path": RANKINGS_DIR / "clf_rankings.parquet",
        "data_dir": Path("paper/data/classification/synthetic"),
    },
    "regression": {
        "rankings_path": RANKINGS_DIR / "reg_rankings.parquet",
...
def main() -> None:
    ...
    for task, config in TASK_CONFIG.items():
        if not config["rankings_path"].exists():
            missing_rankings.append(config["rankings_path"])
            continue
    ...
    if missing_rankings:
        missing = "\n".join(f"  - {path}" for path in missing_rankings)
        raise FileNotFoundError(
            "Synthetic top-ranking tables require local raw ranking caches:\n"
            f"{missing}\n"
            "Regenerate them with aggregate_pipeline_artifacts.py before rebuilding "
            "these support tables."
        )
