...
The guarantee applies to fixed-node, fixed-$B$ tests: the Stage~A feature tests and, in \cref{cor:stageB-maxt}, the max-type Stage~B test for a feature and candidate set fixed without reference to the responses. It does not cover the per-threshold Stage~B search, adaptive stopping, response-dependent feature muting, response-selected internal nodes, bootstrap forests, honesty, or $p$-values after feature selection or other post-selection steps; see
...
The fixed-node results in \cref{sec:theory} use Assumptions A0.1--A0.5. Theorem~\ref{thm:plusone-superuniform} proves $+1$ Monte Carlo permutation $p$-value superuniformity. Corollary~\ref{cor:stageA-global-null} gives Stage~A complete-null control, and Corollary~\ref{cor:stageB-maxt} gives complete-null control for the max-type Stage~B test at a fixed node.
