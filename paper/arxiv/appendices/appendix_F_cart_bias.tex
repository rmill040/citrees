...
mechanism behind ``high-cardinality'' selection bias in greedy CART
...
$q_j := |C_j| \ge 1$. For each $c \in C_j$, let $\Delta_{j,c}$ be a split score
...
\begin{equation}
  ...
  \le q_j \cdot \max_{c \in C_j} \PP(\Delta_{j,c} \ge \delta).
\end{equation}
%
...
%
\begin{equation}
  \PP\!\left(\max_{c \in C_j} \Delta_{j,c} \ge \delta\right) = 1-(1-p_\delta)^{q_j}.
\end{equation}
\end{lemma}
...
$\PP(\max_{c \in C_j}\Delta_{j,c} < \delta) = \prod_{c \in C_j} (1-p_\delta)
  = (1-p_\delta)^{q_j}$, so taking complements yields the identity.
\end{proof}
...
splits is $2^{L-1}-1$ (nonempty proper subsets, modulo swapping left/right).
...
$q_j := |C_j| \ge 1$. Let $S_{j,c}$ denote a split score for candidate split
...
define scores with ties broken as $\widetilde S_{j,c} := (S_{j,c}, V_{j,c})$ ordered
...
%
\begin{equation}
  \PP(\widehat j = j) = \frac{q_j}{\sum_{k=1}^p q_k}.
\end{equation}
\end{lemma}

\begin{proof}
By construction, the augmented collection $\{(S_{j,c},V_{j,c})\}$ is
exchangeable and almost surely has no ties. Therefore the argmin is uniformly
distributed over the $\sum_{k=1}^p q_k$ candidate splits, and it belongs to
feature $j$ with probability $q_j/\sum_{k=1}^p q_k$.
\end{proof}

\begin{remark}[Feature subsampling does not remove the mechanism]
If a greedy CART method first samples a subset of candidate features
...
%
\begin{equation}
  \PP(\widehat j = j \mid F) = \frac{q_j}{\sum_{k \in F} q_k} \qquad (j \in F).
\end{equation}
%
