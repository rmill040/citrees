...
Let $T^{(1)}_b, \dots, T^{(K)}_b$ denote the $K$ selector statistics computed on
permutation $b$ for a fixed feature, and define $M_b := \max_k |T^{(k)}_b|$.
...
measurable function of an exchangeable vector, so the +1 p-value computed from
