...
is at least 0.5, and globally when there are none. The RF is also ranked by
out-of-bag permutation importance, which scores every training row, as an added
comparator.
...
scanning, threshold scanning, and feature muting are each switched off, and the
two corrections are removed together. The 256-bin histogram is replaced by
exact thresholds or, for CIF, by a 32-bin histogram. For CIF, bootstrap
sampling is also switched off, and the max-type rule replaces the per-threshold
rule. Both
...
component at a time. The variants replace fitted split importance with split
counts, disable feature muting, disable bootstrap sampling, or reduce the
forest to one tree. Each variant is evaluated with the same downstream learners,
...
both rules reach. Two reruns check that comparison. One adds a fourth seed with
1,500 null and 1,000 alternative replicates, and the other moves the planted
