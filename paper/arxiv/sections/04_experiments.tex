...
on the held-out fold. We then summarize overall ranks, how often CIF performs
well across datasets, high-dimensional behavior, recovery on synthetic data with
known informative features, CIF ranking ablations, and feature sampling in
forests.
...
excluded (\cref{app:methods}), so the all-method classification panel has 21 of
the 23 datasets, and each pairwise comparison uses the datasets where both
methods are present. The complete-case classification analysis, used for
...
By-$k$ results are reported separately for $k=5,10,25,50,100$, and a dataset
with fewer than $k$ features does not enter the column for $k$. The
corresponding classification dataset counts are $21, 20, 14, 14, 13$; the
regression counts are $8, 8, 7, 7, 6$. Pairwise CIF comparisons stratified by
$k$ use the dataset counts reported with the corresponding rows. The 13-dataset
classification and 6-dataset regression summaries are complete-case analyses,
retaining only datasets with every method, downstream learner, and standard $k$
value. \Cref{tab:panels} names the panels.
...
(RF), and ExtraTrees, using 1,000 trees for forest methods. CIF-all denotes
...
maximum absolute pairwise correlation across 10 random projections. Distance correlation follows
...
methods are CART decision trees (DT), randomized trees (RT) with random split
thresholds, CIT, CIF, random
...
\citep{Guyon2002RFE,Pedregosa2011ScikitLearn}. A further comparator ranks the
RF by out-of-bag permutation importance, which scores every training row.

In this benchmark, CPI uses stratified conditional permutation with
correlation-based strata. It trains one 100-tree random forest on a training
split, scores a 20\% validation split, and treats each feature in turn as the
target feature. It then identifies validation-set features with absolute
...
unconditional variable-importance scores with one permutation, an out-of-bag
permutation importance and the only importance the package provides for
forests; CIF uses split-importance rankings from accepted conditional-inference
splits. The cforest results therefore refer to this benchmarked partykit
importance setting. The two forests thus differ in importance mechanism as well
as in how their trees are grown, and the mechanism-matched check
(\cref{app:cif-perm-check}) ranks CIF by cforest's own importance to separate
the two. Boruta and recursive feature elimination rank by their fitted rank
variables, with smaller ranks first. PI and CPI rank by decreasing importance;
the CPI row uses the stratified conditional-permutation procedure described
above. Ties are broken by deterministic, method-specific order for
reproducibility.
...
evaluated under multiple configurations: four each for CIT and CIF, five for
XGBoost, four for cforest, and two each for LightGBM and ctree. Permutation-test filters used one configuration per statistic. For each
...
regression, each of the 6 complete-case datasets is evaluated using
configurations chosen from the other 5 datasets. The same check is repeated on
the all-method panels. The downstream models, $k$ values, and averaging within
each dataset are unchanged.
...
$100$. The benchmarked CIT and CIF configurations use adaptive sequential
stopping at the minimum budget, the feature and threshold corrections with the
per-threshold rule, feature muting, feature scanning, threshold scanning, and
...
the real datasets of the ablation panel. The ablation holds the selected CIF
configuration fixed and changes one component at a time: replacing fitted split importance with split counts,
...
feature-sampling study of \cref{sec:experiments-datasets}, the weak-signal
cardinality designs, and the support measurement. The weak-signal cardinality
