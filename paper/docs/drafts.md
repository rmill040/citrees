...
paper-facing mathematics and algorithm documentation have been migrated into the
arXiv manuscript source of truth under `paper/arxiv/`.
...
- Algorithm + metrics appendix (implementation-aligned):
...
- Reproducibility + implementation notes:
  `paper/arxiv/appendices/appendix_implementation.tex`
...
3. Final notation pass: ensure consistent symbols across sections (node index
   $t$, feature set $F_t$, thresholds $C_{t,j}$, budgets $B$, etc.).
