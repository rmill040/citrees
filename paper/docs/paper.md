...
This file is the **paper-facing** draft and the single source of truth for the
manuscript’s mathematical statements and supporting derivations. It ties theory
claims to reproducible simulations under `paper/scripts/`.

For a reproducible mapping from scripts → figures/tables → claims, see
`paper/docs/figures_plan.md`. For a concise, paper-ready pseudocode appendix
aligned to the code, see Appendix G. For a detailed, implementation-aligned
methods appendix, see Appendix H. For internal writing notes and the claim
tracker, see Appendix E and Appendix F.
...
5. A reproducibility/audit trail for paper-facing claims: Appendix F records a
   ...
   `paper/docs/figures_plan.md` maps each figure/table to its generating script
   and artifacts.
...
$\min_{c\in C_{t,j_t^\star}} p_{t,c} <
\alpha_{\text{split}}/|C_{t,j_t^\star}|$.
...
so taking complements yields the identity. ∎
...
stopping** modes, the sequential routines also enforce `min_resamples = ⌈1/α'⌉`
by bumping `n_resamples` up if needed.
...
$\mathbb{P}(V\ge r)\le \mathbb{E}[V]/r \le \alpha/r$. ∎
...
Lemma 2 applies directly and yields the bound. ∎
...
the claim follows by set inclusion. ∎
...
with probability $m_j/\sum_k m_k$. ∎
...
1). The proof of Lemma 3b applies verbatim. ∎
...
restricted candidate collection $\{S_{j,c}: j\in F,\ c\in C_j\}$. ∎
...
Under the global null at the root, Proposition 3 gives the bound. ∎
...
Corollary 3d per tree and linearity. ∎
...
2. (Optional) Standardize citations to use `paper/references.bib` keys
   consistently across `paper/*.md`.
...
yields $\mathbb{E}[\widehat{\mu}(L)\mid \Pi]=\mu(L)$ on $\{N\ge 1\}$. ∎
...
- float in $(0,1]$: `ceil(|F_t| * max_features)` (values ≥ 1 are effectively
  “all features”),
- `"sqrt"`: `ceil(sqrt(|F_t|))`,
- `"log2"`: `ceil(log2(|F_t|))`,
...
**Small-sample fallback.** If the feature has very few unique values (≤4), the
implementation uses all midpoints directly, regardless of threshold method.
...
same integer/float/`sqrt`/`log2` rules as `max_features` (floats ≥ 1 effectively
mean “all thresholds”), then applied to the candidate midpoints for the selected
threshold method. When `threshold_method="exact"`, all midpoints are returned
regardless of `max_thresholds` (it is accepted for API compatibility but not
used).
...
based on $T^{\text{max}}$ is super-uniform. ∎
...
**Interpretation:** | $|d|$ Range | Effect Size | |-------------|-------------|
| < 0.2 | Negligible | | 0.2 - 0.5 | Small | | 0.5 - 0.8 | Medium | | ≥ 0.8 |
Large |
