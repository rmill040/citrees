...
def load_real_task_frame(path: Path) -> pd.DataFrame:
    """Load a task parquet and keep only real-data rows."""
    df = pd.read_parquet(path)
    return df[df["dataset_source"] == "real"].copy()
...
def task_global_config_scores(df: pd.DataFrame, metric: str) -> pd.DataFrame:
    ...
    cell_scores = (
        df.groupby(["method_base", "method_id", "dataset", "downstream_model", "k"], as_index=False)[metric]
        .mean()
...
def select_best_task_configs(df: pd.DataFrame, metric: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    filtered = df.merge(best[["method_base", "method_id"]], on=["method_base", "method_id"], how="inner")
...
def dataset_scores(df: pd.DataFrame, metric: str) -> pd.DataFrame:
    ...
    return (
        df.groupby(["downstream_model", "dataset", "k", "method_base", "method_id"], as_index=False)[metric]
        .mean()
        .rename(columns={metric: "dataset_mean_score"})
    )
...
def complete_case_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    complete = counts[counts["n_methods_present"] == n_methods][["downstream_model", "k", "dataset"]]
...
def rank_complete_case_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ranked = df.copy()
    ranked["rank"] = ranked.groupby(["downstream_model", "k", "dataset"])["dataset_mean_score"].rank(
        ascending=False,
        method="average",
    )
    return ranked
