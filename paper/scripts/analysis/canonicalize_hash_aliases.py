...
This script uses the audited alias manifest from ``audit_hash_aliases.py`` to
...
Usage:
  UV_CACHE_DIR=./scratch/.uv_cache uv run python paper/scripts/analysis/canonicalize_hash_aliases.py \
      --local-dir ../data
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Alias manifest CSV generated by audit_hash_aliases.py",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
