...
Usage:
    UV_CACHE_DIR=./scratch/.uv_cache uv run python paper/scripts/analysis/generate_ablation_figures.py
"""
...
def fig_alpha_depth() -> None:
    ...
    df = _read_csv("ablation_block2_alpha.csv")
    if df is None:
        return
...
def fig_n_estimators() -> None:
    ...
    df = _read_csv("ablation_block3_n_estimators.csv")
    if df is None:
        return
...
def fig_scaling() -> None:
    ...
    df = _read_csv("ablation_block4_scaling.csv")
    if df is None:
        return
...
def fig_bootstrap() -> None:
    ...
    df = _read_csv("ablation_block7_feat_subsample.csv")
    if df is None:
        return
...
def fig_nresamples() -> None:
    ...
    df = _read_csv("ablation_block8_nresamples_honesty.csv")
    if df is None:
        return
