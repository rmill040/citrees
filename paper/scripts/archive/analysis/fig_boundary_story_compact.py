...
The figure combines the two boundary claims that now matter in the main paper:
  ...
  2. Sparse-forest diagnostics make sampled-feature exposure the clearest
...
def _load_left_panel() -> pd.DataFrame:
    ...
    heatmap = (
        df.assign(
            downstream_model=pd.Categorical(
                df["downstream_model"], categories=model_order, ordered=True
            ),
...
def main() -> None:
    ...
    gs = fig.add_gridspec(
        2, 2, width_ratios=[1.25, 1.0], height_ratios=[1, 1], wspace=0.18, hspace=0.16
    )
    ...
    ax_left.set_title(
        "A. First $k$ with best CIF score by high-$p$ classification cell",
        fontsize=12,
        loc="left",
        pad=8,
    )
    ...
    legend_handles = [
        Patch(
            facecolor=CATEGORY_COLORS[category], edgecolor="none", label=CATEGORY_DISPLAY[category]
        )
