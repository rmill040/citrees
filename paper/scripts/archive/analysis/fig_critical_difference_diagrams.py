...
Archived exploratory figure helper; not part of the current paper-facing rebuild path.
...
Usage:
    uv run python paper/scripts/archive/analysis/fig_critical_difference_diagrams.py
"""
...
def _find_cliques(avg_ranks: list[float], cd: float) -> list[tuple[float, float]]:
    ...
    for lo, hi in raw:
        if (
            not any(elo <= lo and ehi >= hi and (elo, ehi) != (lo, hi) for elo, ehi in raw)
            and (lo, hi) not in cliques
        ):
