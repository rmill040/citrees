...
Usage:
    uv run python paper/scripts/archive/analysis/fig_dataset_winner_heatmap.py
"""
...
def _load_and_prepare(downstream_model: str) -> pd.DataFrame:
    ...
    global_perf = (
        sub.groupby(["method_base", "method_id"])["balanced_accuracy"].mean().reset_index()
    )
    best_ids = (
    ...
    sub = sub[sub.apply(lambda r: r["method_id"] == best_ids.get(r["method_base"]), axis=1)].copy()

    # Average across seeds and folds
    dataset_means = (
        sub.groupby(["method_base", "dataset", "k"])["balanced_accuracy"].mean().reset_index()
    )
...
def _plot_heatmap(
    ...
    for _, row in winners_df.iterrows():
        ...
        if not np.isnan(cif_rank):
            ...
            text_color = (
                "white" if winner in ("cif", "xgb", "cat", "boruta", "ptest_rdc") else "#1F2937"
            )
            ax.text(
                k_idx,
                ds_idx,
                ...
                va="center",
                fontsize=7,
                fontweight="bold" if rank_int <= 3 else "normal",
                color=text_color,
            )
...
def _print_frequency_table(freq: pd.DataFrame, label: str) -> None:
    ...
    for _, row in freq.iterrows():
        bar = "#" * int(row["pct"] / 2)
        print(
            f"  {DISPLAY_NAMES.get(row['method_base'], row['method_base']):>12s}  "
