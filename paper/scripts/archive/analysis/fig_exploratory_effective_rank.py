...
Usage:
    uv run python paper/scripts/archive/analysis/fig_exploratory_effective_rank.py
"""
...
def _compute_cif_gap() -> pd.DataFrame:
    ...
    sub = df[
        (df["dataset_type"] == "real") & (df["downstream_model"] == "lr") & (df["k"].isin(k_values))
    ].copy()
...
def main() -> None:
    ...
    for i, k in enumerate(k_values):
        ...
        x_line = np.linspace(
            k_sub["log_eff_rank"].min() - 0.05, k_sub["log_eff_rank"].max() + 0.05, 100
        )
        ax.plot(
            x_line,
    ...
    # Plot rho vs k
    ax_traj.plot(
        corr_df["k"], corr_df["rho"], "-o", color="#2563EB", linewidth=2, markersize=6, zorder=3
    )
    ax_traj.axhline(0, color="#9CA3AF", linewidth=0.8, linestyle=":", zorder=1)

    # Significance thresholds
    ax_traj.axhline(
        0.5, color="#16A34A", linewidth=0.8, linestyle="--", alpha=0.5, label=r"$\rho$ = 0.5"
    )
