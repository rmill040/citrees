...
Usage:
    uv run python paper/scripts/archive/analysis/fig_stability_accuracy_scatter.py
"""
...
def make_figure(plot_df: pd.DataFrame, k_display: int) -> None:
    ...
    for _i, row in plot_df.iterrows():
...
def main() -> None:
    ...
    print(
        f"  Methods: {full_df['method_base'].nunique()}, K values: {sorted(full_df['k'].unique())}"
    )
    ...
    for k in sorted(full_df["k"].unique()):
        ...
        correlation_results.append({"k": k, "rho": rho, "pval": pval, "n_methods": len(k_df)})

        print(f"\nk = {k:3d} (n={len(k_df):2d} methods):")
        print(f"  Spearman ρ = {rho:+.3f}, p = {pval:.4f}")
        print(
            f"  Instability range: [{k_df['instability'].min():.3f}, {k_df['instability'].max():.3f}]"
        )
