...
Usage:
    uv run python paper/scripts/archive/analysis/study_cif_config_sensitivity.py
"""
...
def analyze_task(
    ...
    for dm in sorted(cif_cells["downstream_model"].unique()):
        ...
        dm_summary = dm_data.groupby("method_id")[metric].mean().reset_index()
        dm_rank_summary = dm_ranked.groupby("method_id")["rank"].mean().reset_index()
    ...
    print(
        f"  Worst CIF:       {_config_label(config_map[global_summary.loc[global_summary['global_mean'].idxmin(), 'method_id']]):>18s}  {metric}={worst_cif_val:.4f}"
    )
    print(
        f"  Best competitor: {best_competitor_base:>18s}  {metric}={best_competitor_val:.4f}  ({best_competitor_mid})"
    )
    if second_competitor_row is not None:
        print(
            f"  2nd competitor:  {second_competitor_row['method_base']:>18s}  {metric}={second_competitor_row[metric]:.4f}"
        )
    print()
    ...
    for dm in sorted(cif_cells["downstream_model"].unique()):
        dm_means = (
            cif_cells[cif_cells["downstream_model"] == dm].groupby("method_id")[metric].mean()
        )
    ...
    for mid in sorted(config_map.keys()):
        mid_means = (
            cif_cells[cif_cells["method_id"] == mid].groupby("downstream_model")[metric].mean()
        )
