...
Usage:
    uv run python -m paper.scripts.archive.experiments.study_selector_strictness_continuum
"""
...
CIF_CONFIGS: list[tuple[str, dict]] = [
    (
        "strict_default",
        dict(alpha_selector=0.05, adjust_alpha_selector=True, early_stopping_selector="adaptive"),
    ),
    (
        "no_bonf_a05",
        dict(alpha_selector=0.05, adjust_alpha_selector=False, early_stopping_selector="adaptive"),
    ),
    (
        "no_bonf_a10",
        dict(alpha_selector=0.10, adjust_alpha_selector=False, early_stopping_selector="adaptive"),
    ),
    (
        "no_bonf_a20",
        dict(alpha_selector=0.20, adjust_alpha_selector=False, early_stopping_selector="adaptive"),
    ),
    (
        "no_bonf_a50",
        dict(alpha_selector=0.50, adjust_alpha_selector=False, early_stopping_selector="adaptive"),
    ),
    (
        "wide_open_a99",
        dict(alpha_selector=0.99, adjust_alpha_selector=False, early_stopping_selector="adaptive"),
    ),
    (
        "no_bonf_a20_no_es",
        dict(
    ...
    (
        "no_bonf_a05_no_boot",
        dict(
            alpha_selector=0.05, adjust_alpha_selector=False, bootstrap=False, sampling_method=None
        ),
    ),
    ("optimized_a20", dict(alpha_selector=0.20, adjust_alpha_selector=False, max_samples=0.5)),
]
        ...
        for ds_fn in datasets:
            ...
            try:
                ...
                for r_method, r_func, r_params in [
                    ...
                    ("r_cforest_bonf", r_cforest_ranking, dict(testtype="Bonferroni", ntree=100)),
                    (
                        "r_cforest_mc",
                        r_cforest_ranking,
