...
Usage:
    UV_CACHE_DIR=./scratch/.uv_cache uv run python paper/scripts/backlog/analysis/fig_exploratory_feature_agreement.py
"""
...
def _build_agreement_matrix(
    ...
    for i, j in combinations(range(n), 2):
        ...
        if not common_ds:
            continue
        jaccards = [
            _jaccard(consensus[m_a][ds], consensus[m_b][ds])
            for ds in common_ds
        ]
...
def _agreement_accuracy_correlation(
    ...
    # Drop nan pairs
    pairs = [(j, a) for j, a in zip(jac_vals, acc_deltas) if not np.isnan(a)]
    if len(pairs) < 3:
...
def _make_figure(
        ...
        for j in range(n):
            ...
            if i == j:
                ...
                ax.text(j, i, "1.00", ha="center", va="center",
                        fontsize=6, color="#6B7280", fontstyle="italic")
            else:
    ...
    for idx, (xtl, ytl) in enumerate(zip(ax.get_xticklabels(), ax.get_yticklabels())):
...
def main() -> None:
    ...
    for fam in sorted(within, key=lambda f: within[f] if not np.isnan(within[f]) else -1, reverse=True):
        val = within[fam]
    ...
    if pval < 0.05:
        ...
        print(f"      -> Not significant: feature agreement does not predict accuracy similarity.")
