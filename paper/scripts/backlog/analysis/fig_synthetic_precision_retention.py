...
Usage:
    UV_CACHE_DIR=./scratch/.uv_cache uv run python paper/scripts/backlog/analysis/fig_synthetic_precision_retention.py
"""
...
def _right_panel(ax: plt.Axes, agg: pd.DataFrame) -> None:
    ...
    colors = [
        BAR_CAT_COLORS.get(CATEGORY_MAP.get(m, "filter"), "#9CA3AF")
        for m in retention.index
    ]
    display_labels = [DISPLAY_NAMES.get(m, m) for m in retention.index]

    bars = ax.barh(
        range(len(retention)), retention.values,
    ...
    ax.text(
        mean_ret + 0.003, len(retention) - 0.3,
        f"mean={mean_ret:.3f}",
    ...
    ax.legend(
        handles=legend_elements, fontsize=7, loc="lower right",
        framealpha=0.9, edgecolor="#D1D5DB",
    )
