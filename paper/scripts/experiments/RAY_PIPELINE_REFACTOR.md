...
- `S3_BUCKET` must be set on **both head and workers** so Ray tasks can read/write artifacts.
  - Ray EC2 template does this at Ray startup: `paper/scripts/infra/ray/cluster.example.yaml` and
    `paper/scripts/infra/ray/cluster.yaml`.

Recommended environment (for provenance / attribution):
- `GIT_SHA` should be set so every artifact row has a stable code identifier (`git_sha`) even if `.git/` is absent at
  runtime.
  - Ray EC2 template does this at Ray startup: `paper/scripts/infra/ray/cluster.example.yaml` and
    `paper/scripts/infra/ray/cluster.yaml`.
...
- `--datasets <a>,<b>,...`: limit to a few datasets.
...
3. For each fold (currently `N_SPLITS`):
   ...
   - Save:
     ...
     - per-row metadata (repeated each fold for simplicity):
       - `dataset`, `task_type`, `seed`, `method_id`, `method`, `method_base`, `artifact_version`
       - `n_samples`, `n_features`, `selection_cpus`, `elapsed_seconds`, `created_at_utc`, `git_sha`
...
3. For each fold row in rankings parquet:
   ...
   - Each output row is self-describing and includes:
     - `dataset`, `task_type`, `seed`, `method_id`, `method`, `method_base`, `artifact_version`, `fold_idx`, `k`,
       `n_features_selected`, `n_samples`, `n_features`, `evaluation_cpus`, `elapsed_seconds`, `created_at_utc`,
       `git_sha`, `downstream_model`
...
**Status**
- Implemented fix:
  - Metrics rows now include `dataset`, `task_type`, `seed`, `method_id`, `method`, `method_base`, `artifact_version`,
    `n_samples`, `n_features`, `elapsed_seconds`, `created_at_utc`, and `git_sha`.
  - Rankings rows now include `dataset`, `task_type`, `seed`, `method_id`, `method`, `method_base`, `artifact_version`,
    `n_samples`, `n_features`, `selection_cpus`, `elapsed_seconds`, `created_at_utc`, and `git_sha`.
...
- Every row in metrics parquet is “self-describing”, including:
  ...
  - run metadata: `n_samples`, `n_features`, `elapsed_seconds`, `created_at_utc`, `git_sha`, `artifact_version`
  - scheduling metadata: `evaluation_cpus`
...
**Status**
- Implemented: shared helpers now live in `paper/scripts/experiments/_common.py` and are imported by:
  - Stage 1: `paper/scripts/experiments/ray_feature_selection.py`
  - Stage 2: `paper/scripts/experiments/ray_eval.py`
  - Progress: `paper/scripts/experiments/check_progress.py`
...
- `elapsed_seconds` (config-level repeated)

Config-level metadata to add (either repeated in every row or stored in a small companion “manifest”):
- `dataset: str`
- `task_type: str`
- `seed: int`
- `method_id: str` (stable config label; used in S3 key)
- `method: str` (same as `method_id`, kept for compatibility)
- `method_base: str` (base method name, e.g. `cif`)
- `params: dict` (if reasonable) or `params_json: str`
- `n_samples: int`
- `n_features: int`
- `artifact_version: int`
- `created_at_utc: str` (ISO8601)
- `git_sha: str` (best effort; prefer env var `GIT_SHA` for remote runs, otherwise fallback tries `git rev-parse`, else
  `"unknown"`)
...
- identifiers: `dataset`, `task_type`, `seed`, `method_id`, `method`, `method_base`
...
- runtime:
  - `elapsed_seconds` (config-level repeated)
- metadata: `artifact_version`, `created_at_utc`, `git_sha`
- scheduling: `evaluation_cpus`
...
3. Align internal threading with Ray scheduling:
   ...
   - For evaluation, reserve `evaluation_cpus` so configs pack predictably; use it for any downstream model that supports
     `n_jobs`.
...
Stage 2 (evaluation) uses a separate, simple policy:
- `evaluation_cpus_default` is the baseline.
- `evaluation_cpus_overrides` can override specific downstream model names (the effective `evaluation_cpus` is the max
  override across the downstream models used).
...
- dataset helpers:
  ...
  - `get_dataset_path(name, task_type) -> Path`
  - `load_dataset(name, task_type) -> (X, y)`
  - `get_dataset_shape(name, task_type) -> (n_samples, n_features)` (metadata-first, fallback to load)
- artifact path helpers:
  - `rankings_s3_path(task_type, dataset, method_id, seed) -> str`
  - `metrics_s3_path(task_type, dataset, method_id, seed) -> str`
- S3 helpers:
  - `s3_file_exists(s3_path) -> bool` (False only for not-found; otherwise raises)
  - `upload_parquet_to_s3(rows, s3_path) -> None`
  - `download_parquet_from_s3(s3_path) -> pd.DataFrame`

Status:
- Implemented in `paper/scripts/experiments/_common.py`
- Adopted by Stage 1/2 and progress scripts
...
Status:
- Implemented in `paper/scripts/experiments/ray_eval.py` (adds `dataset`, `task_type`, `seed`, `method_id`, `method`,
  `method_base`, `artifact_version`, `n_samples`, `n_features`, `n_features_selected`, `elapsed_seconds`,
  `created_at_utc`, `git_sha`, `evaluation_cpus` to each row).

Acceptance criteria:
- `dataset`, `task_type`, `seed`, `method_id`, `method`, `method_base`, `artifact_version`, `n_samples`, `n_features`,
  `n_features_selected`, `elapsed_seconds`, `created_at_utc`, `git_sha`, `evaluation_cpus` exist as columns in the output.
...
1. Run a smoke test:
   - simplest: `python paper/scripts/experiments/smoke_run.py --source real --seed 0`
   - or manually: Stage 1/2 with `--datasets/--methods/--seeds` filters
2. Compare:
   - artifact sizes,
   - runtime stability,
...
These are ordered by “highest leverage per line of code”.

1. **Make provenance reliable on EC2 (done in Ray cluster template).**
   - `paper/scripts/infra/ray/cluster.example.yaml` and `paper/scripts/infra/ray/cluster.yaml` now set:
     - `S3_BUCKET=...` and `GIT_SHA=...` **inline** in `head_start_ray_commands` and `worker_start_ray_commands`
       (so Ray processes + tasks inherit them).
   - Expected outcome: `git_sha != "unknown"` in both rankings and metrics artifacts.

2. **Add an evaluation CPU policy (done).**
   - Implemented:
     - Config keys: `evaluation_cpus_default` + `evaluation_cpus_overrides` in `paper/scripts/infra/config.yaml`
     - Scheduling: `process_config.options(num_cpus=evaluation_cpus)` in `paper/scripts/experiments/ray_eval.py`
     - Schema: metrics rows include `evaluation_cpus`
   - Notes:
     - Current downstream models (`lr/svm/knn`) are typically light; default remains 1 CPU/config.
     - If you add heavy downstream models later, set overrides (by downstream model name) so Ray packs correctly.

3. **Add a deterministic “smoke run” script (done).**
   - Implemented: `paper/scripts/experiments/smoke_run.py`
   - It runs Stage 1 → Stage 2 for a tiny grid, downloads artifacts from S3, and asserts:
     - required metadata columns exist,
     - `fold_idx` spans `0..N_SPLITS-1`,
     - `git_sha != "unknown"` (provenance is populated).
