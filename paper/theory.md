...
1. **Early stopping inside permutation testing**.
   ...
   - `early_stopping_* = "simple"`: Basic futility + significance stopping (empirically inflates Type I error; not
     used for paper-facing inferential claims)
...
2. **Multi-selector mode** (`selector=[...]`). ✅ **RESOLVED**
   ...
   **Empirical validation (optional).** The validity claim here is already finite-sample (Theorem 1), so simulations
   are only a sanity check. If we include a calibration plot/table for the paper, it should be generated by a
   committed script under `paper/scripts/theory/` and saved into `paper/results/figures/` (so all reported numbers are
   reproducible). For now, we omit hard-coded rejection rates from the draft.
...
**Empirical calibration (null).** We keep empirical results out of theorems, but we do provide reproducible calibration
scripts and paper-ready figures:
...
This method does **not** control Type I error because it "peeks" at the running p-value without proper sequential
adjustment. It is provided only for baseline comparison and research purposes.
...
- The continuous-null PIT argument ($p\sim\mathrm{Unif}(0,1)$) no longer holds exactly, so the equality
  $\mathbb{E}[S_\tau]=\alpha$ is not guaranteed.
- In the sequential Monte Carlo testing literature, ties are handled by **counting ties as losses** (conservative
  p-values). Fischer & Ramdas (2025, Remark 1) note that with this convention their validity results continue to hold.
  Since $W_n:=S_n/\alpha$ is exactly their binomial-mixture wealth process (Section 6.1.3.8), this yields the conservative
  inequality $\mathbb{E}[S_\tau]\le \alpha$ and therefore the same bound
  $\mathbb{P}(S_\tau \ge \gamma)\le \alpha/\gamma$ by Markov/Ville.
...
1. **Permutation p-value validity.**
   ...
   - Avoid `early_stopping_* = "simple"` for inferential claims (inflates Type I error)
