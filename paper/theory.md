...
**Key Observation:** citrees uses a slightly lower threshold ($\gamma/\alpha = 19$) compared to
Fischer-Ramdas ($1/\alpha = 20$). This explains:
1. Why citrees is slightly more aggressive (4/18 dominance violations in empirical tests)
2. Why citrees is faster (lower threshold → earlier stopping)
3. Why Type I error is still controlled (difference is small enough to maintain validity)

#### 6.1.3.3 Potential Paths to Formal Proof

**Path 1: Direct Ville's Inequality Application**

If citrees used threshold $1/\alpha$ instead of $\gamma/\alpha$, Ville's inequality would directly
apply via the Fischer-Ramdas proof. The question is: what happens with the lower threshold?

**Conjecture 1:** For $\gamma$ sufficiently close to 1, the citrees stopping rule controls
Type I error at level:
$$
\mathbb{P}_{H_0}(\text{reject}) \leq \alpha \cdot \frac{1}{\gamma}
$$

For $\gamma = 0.95$, this gives $\alpha/0.95 \approx 1.053\alpha$, a slight inflation.
...
#### 6.1.3.8 Supermartingale Analysis (RESOLVED)

> **Result:** $S_n$ is **NOT** a supermartingale, but Type I error is still controlled via
> Bayesian calibration + Markov inequality.

**Verification (from `paper/scripts/theory/supermartingale_check.py`):**

We checked whether $\mathbb{E}[S_{n+1} | L_n] \leq S_n$ holds universally. Counterexamples exist:
for some $(L_n, n)$ pairs, the expected value increases. This means Ville's inequality cannot
be applied directly.

**However, the key result holds:**

$$\mathbb{E}[S_n] = \alpha \quad \text{exactly, for any fixed } n$$

**Proof:** Under $H_0$, the prior $p \sim \text{Uniform}(0, 1)$ is correct (by probability integral
transform). The posterior $p | L_n \sim \text{Beta}(1 + L_n, 1 + n - L_n)$ is therefore well-calibrated:
$$
\mathbb{E}[S_n] = \mathbb{E}[P(p < \alpha | L_n)] = P(p < \alpha) = \alpha
$$

This is Bayesian calibration with a correct prior. ∎

**Empirical Verification (N = 10,000 simulations):**

| Quantity | Value | Interpretation |
|----------|-------|----------------|
| $\mathbb{E}[S_n]$ for fixed $n$ | 0.0500 | Exact match with $\alpha$ |
| $\mathbb{E}[S_\tau]$ at stopping | 0.0499 | Slightly below $\alpha$ |
| Type I Error | 0.0448 | Below Markov bound |
| Markov Bound ($\alpha/\gamma$) | 0.0526 | Upper bound satisfied |
...
4. **No martingale theory required.**
   We do not need $S_n$ to be a martingale or supermartingale. The tower property alone is
   sufficient.
...
The elegance of this proof becomes apparent when contrasted with approaches that **do not work**:

**1. Supermartingale approach (failed):**

We showed in Section 6.1.3.8 that $S_n$ is **not** a supermartingale:
$\mathbb{E}[S_{n+1} | L_n] \not\leq S_n$ in general. Counterexamples exist where the expected
value increases. Therefore, Ville's inequality cannot be applied directly.
...
- **Ville's inequality** provides Type I error control for test supermartingales, but requires
  the supermartingale property which $S_n$ does not satisfy.
...
1. ~~**Supermartingale Property**~~ **RESOLVED:** $S_n$ is not a supermartingale, but this is not
   required for Type I error control.
