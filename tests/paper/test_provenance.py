...
def test_ranking_artifact_records_complete_r_execution_provenance(
    ...
    assert store.saved["fold_random_state"].tolist() == [7000, 7001, 7002, 7003, 7004]
    assert store.saved["git_sha"].unique().tolist() == ["a" * 40]
    assert store.saved["container_image"].unique().tolist() == ["repository@sha256:" + "a" * 64]
    assert store.saved["dataset_sha256"].unique().tolist() == ["d" * 64]
    assert store.saved["method_params_json"].unique().tolist() == [
        '{"alpha":0.05,"testtype":"Bonferroni"}'
    ]
    assert store.saved["artifact_prefix"].unique().tolist() == ["repairs/run-001"]
    assert store.saved["campaign_sha256"].unique().tolist() == ["e" * 64]
    assert store.saved["manifest_sha256"].unique().tolist() == ["b" * 64]
    assert store.saved["aws_account_id"].unique().tolist() == ["123456789012"]
    assert all(
        value
        == {
            "python": "3.12.7",
        ...
        for value in store.saved["library_versions"]
    )
    assert all(
        value == {"logical_cpus": 32, "ec2_instance_type": "c6a.8xlarge"}
        for value in store.saved["hardware"]
    )
    ...
    class ExistingStore:
        ...
        def load(
            ...
            return store.saved.assign(manifest_sha256="c" * 64)
